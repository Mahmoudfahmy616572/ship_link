...
import 'package:ship_link/driver/presentation/screens/DriverProfile/components/driver_charts_section.dart';
...
import 'package:ship_link/driver/presentation/cubits/upDateUserData/up_date_user_data_cubit.dart';
...
class _BodyState extends State<Body> {
  final _chartsKey = ValueNotifier<int>(0);
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          ValueListenableBuilder<int>(
                            valueListenable: _chartsKey,
                            builder: (context, chartsKey, _) {
                              return DriverChartsSection(key: ValueKey(chartsKey));
                            },
                          ),
                          SizedBox(height: 24.h),
                          _buildMenuItem(
  ...
  Future<Map<String, double>> _getAllEarnings(String driverId) async {
    ...
    try {
      final results = await Future.wait([
        svc.getTodayEarnings(driverId).then((r) => r.fold((_) => 0.0, (v) => v)),
        svc.getWeekEarnings(driverId).then((r) => r.fold((_) => 0.0, (v) => v)),
        svc.getAllTimeEarnings(driverId).then((r) => r.fold((_) => 0.0, (v) => v)),
      ]);
