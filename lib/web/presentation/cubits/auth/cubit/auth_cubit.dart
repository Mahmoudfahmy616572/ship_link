...
export 'package:ship_link/web/presentation/cubits/auth/cubit/auth_stat.dart';
...
class AuthCubit extends Cubit<AuthState> {
  ...
  void _onAuthStateChange(gotrue.AuthState authState) {
    ...
    if (authState.event == gotrue.AuthChangeEvent.signedIn && !_googleInProgress) {
      if (_supabase.auth.currentUser != null) {
        _handleUserSession(_supabase.auth.currentUser!);
      }
      return;
    }
  ...
  Future<void> _startGoogleSignIn() async {
    ...
    try {
      ...
      if (!launched) {
        _googleInProgress = false;
        emit(ErrorState('Popup blocked. Please allow popups for this site.'));
      }
    } catch (e) {
      _googleInProgress = false;
