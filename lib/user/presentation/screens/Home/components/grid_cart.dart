...
class _CartButtonState extends State<_CartButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final inCart = context.select<GetFromCartCubit, bool>(
      (c) => c.state is GetFromCartSuccess &&
          (c.state as GetFromCartSuccess).getProductFromCart.details
              ?.any((d) => d.product?.id == productId) == true);

    return ValueListenableBuilder<bool>(
      valueListenable: _optimisticAdd,
      builder: (context, optimistic, _) {
        final showCheck = inCart || optimistic;
        return GestureDetector(
          onTap: () async {
            if (showCheck) {
              final state = context.read<GetFromCartCubit>().state;
              if (state is GetFromCartSuccess) {
                final detail = state.getProductFromCart.details
              ...
              await context.read<AddToCartCubit>().addToCart(id: productId);
              if (!context.mounted) return;
