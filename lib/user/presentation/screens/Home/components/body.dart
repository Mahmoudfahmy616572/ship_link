...
class _BodyState extends State<Body> {
  ...
  List<Product> _sorted(List<Product> products, String sortBy) {
    if (sortBy.isEmpty) return products;
  ...
  List<Product> get _displayProducts {
    ...
    if (filterState.selectedCategories.isEmpty) {
      return _sorted(all, filterState.sortBy).take(filterState.visibleCount).toList();
    }
    final filtered = all.where((p) => p.category != null && filterState.selectedCategories.contains(p.category)).toList();
    return _sorted(filtered, filterState.sortBy).take(filterState.visibleCount).toList();
  }
  ...
  int get _totalCount {
    final selectedCategories = context.read<HomeFilterCubit>().state.selectedCategories;
    if (selectedCategories.isNotEmpty) {
      return _allProducts.where((p) => p.category != null && selectedCategories.contains(p.category)).length;
    }
    return _allProducts.length;
  }

  bool get _hasMore => context.read<HomeFilterCubit>().state.visibleCount < _totalCount;
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 300) {
      if (_hasMore) context.read<HomeFilterCubit>().loadMore();
    }
  }
                    ...
                    child: Column(
                      children: [
                        ...
                        FavouriteBrands(
                          ...
                          selectedCategory: filterState.selectedCategories.isEmpty ? null : filterState.selectedCategories.first,
                        ...
                        SizedBox(height: 28.h),
                        _SectionHeader(
                          title: filterState.selectedCategories.isNotEmpty ? context.t.tr('filtered_products') : context.t.tr('all_products'),
                          trailing: filterState.selectedCategories.isNotEmpty ? context.t.tr('clear_filter') : null,
                          onTrailingTap: filterState.selectedCategories.isNotEmpty ? _clearFilter : null,
                        ),
                        SizedBox(height: 14.h),
                        _SortBar(
